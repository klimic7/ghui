...
export const detectCurrentGitHubRepository = (cwd = initialWorkingDirectory): string | null => {
	const remotes = Bun.spawnSync({ cmd: ["git", "-C", cwd, "remote"], stdout: "pipe", stderr: "pipe" })
	...
	for (const name of orderedNames) {
		const url = Bun.spawnSync({ cmd: ["git", "-C", cwd, "remote", "get-url", name], stdout: "pipe", stderr: "pipe" })
